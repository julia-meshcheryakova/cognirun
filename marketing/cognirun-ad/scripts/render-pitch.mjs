...
try {
  ...
  if (composition.durationInFrames !== 3600 || composition.fps !== 30) throw new Error('Pitch composition must be 120 seconds at 30fps.');
  ...
  for (const [name, frame] of [['opening', 100], ['voice-answer', 1450], ['results', 2750], ['closing', 3300]]) await renderStill({serveUrl, composition, frame, output: path.join(qa, `pitch-${name}.png`), imageFormat: 'png', puppeteerInstance: browser, browserExecutable, logLevel: 'warn'});
  console.log('Rendered out/cognirun-pitch-16x9.mp4 (1920x1080, 120 seconds).');
